...
if ACCESS_KEY_ID == "" or SECRET_ACCESS_KEY == "":
    print("请提供火山引擎的 App Key 和 Secret Key!可从[火山引擎控制台](https://console.volcengine.com/iam/keymanage)获取～")
    exit(1)
# 请求地址
...
def submit(args: argparse.Namespace):
    ...
    try:
        ...
        if not code or code[0] != 0:
            output_error(code and code[0], f"提交任务失败, Code: {code and code[0]}")
            exit(1)

        task_id = jsonpath.jsonpath(response, "$.Result.TaskId")
        if not task_id or not task_id[0]:
            output_error(-1, f"解析TaskId失败, 响应内容: {response.text}")
            exit(1)
        return task_id[0]
    except Exception as e:
...
def poll(task_id: str):
    ...
    try:
        for _ in range(POLL_MAX_ATTEMPTS):
            ...
            output_error(result_code and result_code[0], f"查询任务状态失败, Code: {result_code and result_code[0]}")
            exit(1)
...
def register_ark_claw_combo():
    ...
    try:
        ...
        if open_top_code and open_top_code[0] != 0:
            output_error(open_top_code[0], f"查询套餐信息失败, Code: {open_top_code[0]}")
            exit(1)
        ...
        if code and code[0] != 0:
            output_error(code[0], f"查询套餐信息失败, Code: {code[0]}")
            exit(1)
